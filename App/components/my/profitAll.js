...
export default class profitAll extends Component {
            ...
            >
                <View style={styles.EarningsItemBox}>
                    ...
                    <View style={styles.itemRight}>
                        <Text
                            style={[styles.itemPrice, {color: item.price < 0 ? '#212121' : '#2ecd32'}]}>{item.price > 0 ? '+' + item.price : item.price}元</Text>
    ...
    incomeList() {
        ...
        fetch(requestUrl.detailList + `?pageSize=${this.state.pageSize}&pageNo=1`)
    ...
    incomeListMore(pageNo) {
        fetch(requestUrl.detailList + `?pageSize=${this.state.pageSize}&pageNo=${pageNo}`)
            .then((response) => response.json())
            .then((responseData) => {
                console.log(responseData);
